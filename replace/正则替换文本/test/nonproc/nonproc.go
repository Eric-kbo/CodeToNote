...
func InspectNonprocExport(ctx *gin.Context) {
	...
	case 1:
		sheetName = "PGC普通抽检单查询列表"
		bs, err = nonprocinvoker.InspectPgcNonprocExport(result.Data, sheetName)
		break
	case 2:
		if request.TypeID == 2 {
			sheetName = "UGC图文普通抽检单查询列表"
			bs, err = nonprocinvoker.InspectUgcArticleNonprocExport(result.Data, sheetName)
		} else {
			sheetName = "UGC普通抽检单查询列表"
			bs, err = nonprocinvoker.InspectUgcNonprocExport(result.Data, sheetName)
		}
		break
	case 3:
		sheetName = "UGC优先抽检单查询列表"
		bs, err = nonprocinvoker.InspectUgcNonprocExport(result.Data, sheetName)
		break
	case 4:
		sheetName = "PGC优先抽检单查询列表"
		bs, err = nonprocinvoker.InspectPgcNonprocExport(result.Data, sheetName)
		break
	case 5:
		if request.TypeID == 2 {
			sheetName = "UGC安全图文普通抽检单查询列表"
			bs, err = nonprocinvoker.SafeUgcArticleNonprocExport(result.Data, sheetName, true)
		} else {
			sheetName = "UGC安全视频普通抽检单查询列表"
			bs, err = nonprocinvoker.SafeUgcVideoNonprocExport(result.Data, sheetName, true)
		}
		break
	case 6:
		if request.TypeID == 2 {
			sheetName = "UGC安全图文优先抽检单查询列表"
			bs, err = nonprocinvoker.SafeUgcArticleNonprocExport(result.Data, sheetName, true)
		} else {
			sheetName = "UGC安全视频优先抽检单查询列表"
			bs, err = nonprocinvoker.SafeUgcVideoNonprocExport(result.Data, sheetName, true)
		}
		break
	case 12:
		if request.TypeID == 1 {
			sheetName = "视频号任务激励抽检单查询列表"
			bs, err = nonprocinvoker.SafeUgcVideoNonprocExport(result.Data, sheetName, true)
		}
		break
	}
...
func QualityNonprocExport(ctx *gin.Context) {
	...
	switch request.CategoryID {
	case 1:
		sheetName = "PGC普通质检单查询列表"
		bs, err = nonprocinvoker.QualityPgcNonprocExport(result.Data, sheetName)
		break
	case 2:
		if request.TypeID == 2 {
			sheetName = "UGC图文普通质检单查询列表"
			bs, err = nonprocinvoker.QualityUgcArticleNonprocExport(result.Data, sheetName)
		} else {
			sheetName = "UGC普通质检单查询列表"
			bs, err = nonprocinvoker.QualityUgcNonprocExport(result.Data, sheetName)
		}
		break
	case 3:
		sheetName = "UGC优先质检单查询列表"
		bs, err = nonprocinvoker.QualityUgcNonprocExport(result.Data, sheetName)
		break
	case 4:
		sheetName = "PGC优先质检单查询列表"
		bs, err = nonprocinvoker.QualityPgcNonprocExport(result.Data, sheetName)
		break
	case 5:
		if request.TypeID == 2 {
			sheetName = "UGC安全图文普通质检单查询列表"
			bs, err = nonprocinvoker.SafeUgcArticleNonprocExport(result.Data, sheetName, false)
		} else {
			sheetName = "UGC安全视频普通质检单查询列表"
			bs, err = nonprocinvoker.SafeUgcVideoNonprocExport(result.Data, sheetName, false)
		}
		break
	case 6:
		if request.TypeID == 2 {
			sheetName = "UGC安全图文优先质检单查询列表"
			bs, err = nonprocinvoker.SafeUgcArticleNonprocExport(result.Data, sheetName, false)
		} else {
			sheetName = "UGC安全视频优先质检单查询列表"
			bs, err = nonprocinvoker.SafeUgcVideoNonprocExport(result.Data, sheetName, false)
		}
		break
	case 12:
		if request.TypeID == 1 {
			sheetName = "视频号任务激励质检单查询列表"
			bs, err = nonprocinvoker.SafeUgcVideoNonprocExport(result.Data, sheetName, false)
		}
		break
	}
